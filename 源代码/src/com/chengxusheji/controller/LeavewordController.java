...
public class LeavewordController extends BaseController {
	...
	public void studentAdd(Leaveword leaveword, BindingResult br,
	...
	/*ajax方式按照查询条件分页查询留言答疑信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String leaveTitle,@ModelAttribute("userObj") Student userObj,String leaveTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public String frontlist(String leaveTitle,@ModelAttribute("userObj") Student userObj,String leaveTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
	...
	public String studentFrontlist(String leaveTitle,@ModelAttribute("userObj") Student userObj,String leaveTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
	    ...
	    List<Student> studentList = studentService.queryAllStudent();
	    request.setAttribute("studentList", studentList);
		return "Leaveword/leaveword_studentFrontquery_result"; 
	}
	...
	public String delete(@PathVariable Integer leaveWordId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  leavewordService.deleteLeaveword(leaveWordId);
	            request.setAttribute("message", "留言答疑删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "留言答疑删除失败!");
				return "error";
	...
	/*按照查询条件导出留言答疑信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String leaveTitle,@ModelAttribute("userObj") Student userObj,String leaveTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(leaveTitle == null) leaveTitle = "";
        if(leaveTime == null) leaveTime = "";
        ...
        String[] headers = { "留言id","留言问题","留言人","留言时间","答疑回复","答疑时间"};
